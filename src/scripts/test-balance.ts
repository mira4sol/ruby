...
async function main() {
  const label = 'general' // Adjust to 'GAS' or 'SAVINGS' as needed
  console.log(`Fetching balance for wallet: ${label}...`)

  try {
    const response = await fetch(`${BASE_URL}/wallets/${label}/balance`, {
      method: 'GET',
      headers: {
        'x-api-key': API_KEY,
