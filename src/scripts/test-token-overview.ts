...
async function main() {
  // Example token: JUP
  const mintAddress = 'JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN'
  console.log(`Fetching overview for token: ${mintAddress}...`)

  try {
    const response = await fetch(`${BASE_URL}/tokens/${mintAddress}/overview`, {
      method: 'GET',
      headers: {
        'x-api-key': API_KEY,
