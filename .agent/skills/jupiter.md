...
description: Comprehensive guidance for integrating Jupiter APIs (Ultra Swap,
  Lend, Perps, Trigger, Recurring, Tokens, Price, Portfolio, Prediction Markets,
  Send, Studio, Lock, Routing). Use for endpoint selection, integration flows,
...
async function jupiterAction<T>(
  action: () => Promise<T>,
): Promise<JupiterResult<T>> {
  try {
    ...
    if (typeof code === 'number' && code < 0) {
      const retryable = [
        -1, -1000, -1001, -1005, -1006, -2000, -2003, -2005,
      ].includes(code)
      return {
        ok: false,
        error: { code, message: error?.error ?? 'Execute failed', retryable },
      }
    }
    ...
    if (typeof code === 'number' && code > 0) {
      return {
        ...
        error: {
          code,
          message: error?.error ?? 'Program error',
          retryable: false,
        },
      }
    }
